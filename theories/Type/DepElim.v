...
Lemma Id_symmetry_dep {A} {t : A} {B : forall (x : A), x = t -> Type} :
  (forall (x : A) (eq : t = x), B x (id_sym eq)) ->
  (forall (x : A) (eq : x = t), B x eq).
Proof.
  intros. rewrite <- Id_sym_invol.
  generalize (id_sym eq). apply X.
Defined.
...
Create HintDb equations.

#[global]
Hint Unfold solution_left solution_right
  Id_sym_invol Id_symmetry_dep
...
Extraction Inline Id_sym_invol Id_symmetry_dep.
