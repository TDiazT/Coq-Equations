...
Import Sigma_Notations Id_Notations.
...
Section TeleSigma.
  ...
  | tip A, P := P
  | ext A B, P := fun x => tele_pred_pack (B x.1) (P x.1) x.2.
...
Instance wf_tele_measure@{i j k +| i <= k, j <= k +}
         {T : tele@{i}} (A : Type@{j}) (f : tele_fn@{i j k _} T A) (R : A -> A -> Type@{k}) :
...
Section Fix.
  ...
  Context (P : tele_type@{i k l _} T).

  (* (forall x : A, (forall y : A, R y x -> P y) -> P x) -> forall x : A, P x *)
  Definition tele_fix_functional_type :=
    tele_forall_uncurry@{i m m _ _} T (fun x =>
      ((tele_forall_uncurry@{i m m _ _} T (fun y =>
...
Section FixUnfold.
  ...
  (* (forall x : A, (forall y : A, R y x -> P y) -> P x) -> forall x : A, P x *)
  Context (fn : tele_fix_functional_type@{i j k l m _ _ _} R P).
  ...
  Proof.
    ...
    rewrite tele_forall_app_type@{i k l _ _}. destruct (wellfounded x). simpl.
